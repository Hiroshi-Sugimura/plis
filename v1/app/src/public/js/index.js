...
function onLoad() {
	...
	window.ipc.on('to-renderer', (event, obj) => {
		...
		try {
			...
			console.error('index.js to-renderer error:', error);
			console.error('main -> error cmd:', c.cmd, "arg:", c.arg);
			window.addError({ datetime: window.getToday(), moduleName: c.cmd, stackLog: `${error.message}` });
		}
	});
