...
let mainEL = {
	...
	observation: async function () {
		...
		if (network.IPver == '0' || network.IPver == '4' || network.IPver == 0 || network.IPver == 4) {
			...
			await EL.sendOPC1(EL.Multi, [0x0e, 0xf0, 0x01], [0x02, 0x65, 0x00], EL.GET, [0x80, 0xD0, 0xD1, 0xE0, 0xE1, 0xE3, 0xEA], [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);  // 電動窓
		...
		if (!mainEL.disableIPv6 && (network.IPver == '0' || network.IPver == '6' || network.IPver == 0 || network.IPver == 6)) {
			...
			await EL.sendOPC1(EL.Multi6, [0x0e, 0xf0, 0x01], [0x02, 0x65, 0x00], EL.GET, [0x80, 0xD0, 0xD1, 0xE0, 0xE1, 0xE3, 0xEA], [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);  // 電動窓
