...
window.addEventListener('DOMContentLoaded', function () {
	...
	// Netatmo認証・通信エラー通知（mainからIPCで受け取る）
	if (window.ipc && window.ipc.on) {
		window.ipc.on('NetatmoAuthError', function (msg) {
			window.addToast('Error', 'Netatmo認証エラー: ' + (msg || '認証情報（Refresh Token）が無効です。Netatmo開発者ポータルで新しいRefresh Tokenを生成し、アプリの設定画面で再設定を行ってください。'));
		});
		window.ipc.on('NetatmoConnectionError', function (msg) {
			window.addToast('Warning', 'Netatmo通信エラー: ' + (msg || 'Netatmoサーバーとの通信に一時的に失敗しました。インターネット接続環境、またはNetatmoのサーバー稼働状況を確認してください（自動で再試行を継続します）。'));
		});
	}
