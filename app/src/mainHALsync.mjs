...
let mainHALsync = {
    ...
    //----------------------------------------------------------------------------------------------
    /**
     * @func migrateGarminStressDetailsTable
     * @desc データベーステーブルのマイグレーション処理
     * @async
     */
    migrateGarminStressDetailsTable: async function() {
        try {
            // sqlite3のインポートが必要
            const { sqlite3 } = await import('./models/localDBModels.cjs');
            
            // 新しいカラムが存在するかチェック
            const tableInfo = await sqlite3.query("PRAGMA table_info(IOT_GarminStressDetails)", {
                type: sqlite3.QueryTypes.SELECT
            });
            
            const hasBodyBatteryDynamic = tableInfo.some(col => col.name === 'bodyBatteryDynamicFeedbackEvent');
            const hasBodyBatteryActivity = tableInfo.some(col => col.name === 'bodyBatteryActivityEventList');
            
            if (!hasBodyBatteryDynamic) {
                await sqlite3.query("ALTER TABLE IOT_GarminStressDetails ADD COLUMN bodyBatteryDynamicFeedbackEvent TEXT", {
                    type: sqlite3.QueryTypes.RAW
                });
                console.log('✓ Added bodyBatteryDynamicFeedbackEvent column');
            }
            
            if (!hasBodyBatteryActivity) {
                await sqlite3.query("ALTER TABLE IOT_GarminStressDetails ADD COLUMN bodyBatteryActivityEventList TEXT", {
                    type: sqlite3.QueryTypes.RAW
                });
                console.log('✓ Added bodyBatteryActivityEventList column');
            }
    ...
    startSync: async function () {
        config.debug ? console.log(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), '| mainHALsync.startSync().') : 0;

        // === データベースマイグレーションを実行 ===
        try {
            await mainHALsync.migrateGarminStressDetailsTable();
        } catch (error) {
    ...
    garminDownload: async function () {
        config.debug ? console.log(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), '| mainHALsync.garminDownload().') : 0;
        ...
        try {
            ...
            if (dndata.BodyComps) {
                ...
                config.debug ? console.log(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), '|- Inserted a record in the IOT_GarminBodyCompsModel teble.') : 0;
            } else {
                config.debug ? console.log(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), '|- No record in the IOT_GarminBodyCompsModel teble.') : 0;
            }
            ...
            if (dndata.StressDetails) {
                ...
                await IOT_GarminStressDetailsModel.findOrCreate({
                    ...
                    defaults: {
                        ...
                        // === 新しいフィールドを追加 ===
                        bodyBatteryDynamicFeedbackEvent: JSON.stringify(dndata.StressDetails.bodyBatteryDynamicFeedbackEvent || null),
                        bodyBatteryActivityEventList: JSON.stringify(dndata.StressDetails.bodyBatteryActivityEventList || []),
                        
                        createdAt: dndata.StressDetails.createdAt,
                        updatedAt: dndata.StressDetails.updatedAt
                    }
                });
                config.debug ? console.log(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), '|- Inserted a record in the IOT_GarminStressDetailsModel table.') : 0;
            } else {
                config.debug ? console.log(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), '|- No record in the IOT_GarminStressDetailsModel table.') : 0;
            }
