...
let mainHALlocal = {
    ...
    getLastData: async function () {
        ...
        // =====【Garminデータ取得処理を追加】=====
        // Garmin関連テーブルからデータを取得
        let garminData = {};
        
        try {
            // 各Garminテーブルから最新データを取得
            const garminTables = [
                'IOT_GarminActivities',
                'IOT_GarminActivityDetails', 
                'IOT_GarminBodyComps',
                'IOT_GarminDailies',
                'IOT_GarminEpochs',
                'IOT_GarminMoveIQActivities',
                'IOT_GarminPulseox',
                'IOT_GarminSleeps',
                'IOT_GarminStressDetails',
                'IOT_GarminUserMetrics'
            ];
            
            for (const tableName of garminTables) {
                try {
                    // 各テーブルから最新レコードを取得（createdAtの降順で1件）
                    const query = `
                    ...
                    const results = await sqlite3.query(query, {
                        type: sqlite3.QueryTypes.SELECT
                    });
                    
                    if (results && results.length > 0) {
                        // テーブル名から接頭辞を除去してキー名を作成
                        const key = tableName.replace('IOT_Garmin', '');
                        garminData[key] = results[0];
                        config.debug ? console.log(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), `| mainHALlocal.getLastData() ${key} data found:`, results[0].calendarDate || 'No date') : 0;
                    } else {
                        const key = tableName.replace('IOT_Garmin', '');
                        garminData[key] = null;
                        config.debug ? console.log(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), `| mainHALlocal.getLastData() ${key} data not found`) : 0;
                    }
                } catch (tableError) {
                    const key = tableName.replace('IOT_Garmin', '');
                    garminData[key] = null;
                    console.error(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), `| mainHALlocal.getLastData() Error fetching ${key}:`, tableError.message);
                }
            }
            
            config.debug ? console.log(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), '| mainHALlocal.getLastData() Garmin data collection completed') : 0;
            
        } catch (error) {
            console.error(new Date().toFormat("YYYY-MM-DDTHH24:MI:SS"), '| mainHALlocal.getLastData() Garmin data fetch error:', error);
        }
        // ==========================================

        halData.minorkeyMeans = MinorkeyMeans;

        return { MajorResults, MinorResults, MinorkeyMeans, GarminData: garminData };
    },
